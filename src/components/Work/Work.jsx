...
const Work = () => {
      ...
      <div className="flex flex-wrap justify-center  gap-6  w-full">
        {projects.map((project) => (
          <div
            ...
            className="border w-[400px] md:w-[500px] flex flex-col justify-between p-5 border-white bg-gray-900 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden cursor-pointer hover:shadow-purple-500/50 hover:-translate-y-2 transition-transform duration-300"
          >
            <div className="">
             <Carousel
  ...
  emulateTouch={true}     // allow swipe
  selectedItem={0}        // start with first image
>
  {(Array.isArray(project.image) ? project.image : [project.image]).map((img, index) => (
    <div key={index} className="overflow-hidden rounded-xl ">
      <img
        src={img}
        alt={`${project.title} ${index}`}
        className="w-full h-64 object-cover rounded-xl transform transition-transform duration-500 hover:scale-110 cursor-zoom-in"
      />
    </div>
  ))}
</Carousel>
